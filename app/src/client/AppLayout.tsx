...
export default function AppLayout({ user, children }: AppLayoutProps) {
  ...
  // Unread prospect SMS replies. Polled so a reply that arrives while the app is
  ...
  const { data: smsAlerts } = useQuery(getSmsReplyAlerts, undefined, {
    ...
    // A reply that landed while the tab was in the background should be there
    // the moment the user comes back, without waiting for the next tick.
    refetchOnWindowFocus: true,
  });
  const smsReplyCount = (smsAlerts as any)?.total ?? 0;
  const [mobileOpen, setMobileOpen] = useState(false);
  const navigate = useNavigate();
  const location = useLocation();

  // Toast when the unread count grows, so a reply is noticed even on a page that
  // has nothing to do with prospection. The first load only primes the counter —
  // replies already waiting when the app opened are not "new".
  const prevSmsReplyCount = useRef<number | null>(null);
  useEffect(() => {
    if (smsAlerts === undefined) return;
    const prev = prevSmsReplyCount.current;
    prevSmsReplyCount.current = smsReplyCount;
    if (prev === null || smsReplyCount <= prev) return;
    const added = smsReplyCount - prev;
    toast(
      t => (
        <button
          className='text-left'
          onClick={() => { toast.dismiss(t.id); navigate('/prospection'); }}
        >
          <span className='font-medium'>
            {added} nouvelle{added > 1 ? 's' : ''} réponse{added > 1 ? 's' : ''} SMS
          </span>
          <span className='block text-xs text-muted'>Cliquez pour voir la prospection</span>
        </button>
      ),
      { icon: '💬', duration: 8000 },
    );
  }, [smsAlerts, smsReplyCount, navigate]);
